...
simulation <- arrow::read_parquet("data/main_2021-09-05/summary_simulation_adp.parquet") %>% 
  ...
  mutate(
    ...
    franchise_name = map2_chr(franchise_name,franchise_url,~as.character(htmltools::tags$a(.x,href = .y))),
...
crosstalk_simulation %>% 
  reactable(
    ...
    width = 'auto',
    columns = list(
      `Franchise Name` = colDef(minWidth = 150,sticky = "left", html = TRUE),
...
crosstalk_satellite %>% 
  reactable(
    ...
    width = 'auto',
    columns = list(
      `Franchise Name` = colDef(minWidth = 150,html = TRUE,sticky = "left"),
